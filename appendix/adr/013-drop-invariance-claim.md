...
tautology.** In `lifetime()` the drag term is `-0.5 * rho(h, scale) * v**2 / BC`, so `scale`
...
of failing.** The ×1.80 point value survives at mean and high activity; the invariance does
not.

## Validation
GMAT, three activity levels, propagated to the 120 km floor. The mechanism was tested rather
than guessed: sweeping `astro.py`'s density scale over 40× moves the multiplier only
1.7992 → 1.7968. Closing the BC half needs GMAT at BC 40 and 90 — until then the honest

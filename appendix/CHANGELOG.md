...
"belongs in the flagship and the solver output does not", on the strength of a sentence in
`validation/README.md` telling contributors to commit input decks. `.gitignore` had already
decided the opposite, under the comment that the decks are regenerated by
`validation/fea/build_deck.py`. The entry itself called that answering the question by accident.

Settled by measuring the generator rather than by choosing the more agreeable of the two texts.
`build_deck.py` reads three tracked files and nothing else. Run twice in the same environment it
produced byte-identical decks, 2 252 204 bytes pinned and 2 252 205 bytes clamped. Solving both
with CalculiX 2.21-1 under gmsh 4.15.2 reproduced A4's published bracket:

| Case | Reproduced | Published |
|---|---:|---:|
| pinned | 0.01945 mm | 0.0194 mm |
| clamped | 0.01600 mm | 0.0160 mm |

A deck that is a deterministic function of committed inputs carries no provenance its generator
does not already carry, so committing 4.5 MB of it buys nothing, and ADR-015's derive-never-paste
rule applies. `validation/README.md` and `docs/RELATED_WORK.md` now state the rule that way:
commit the generator where a tracked one exists, commit the deck where none does, commit result
JSON either way. What the measurement does not cover is gmsh version drift, which is why the run
sheet records mesher and solver versions beside the hashes, so a later mismatch is visible rather
than silent.

The stray files the entry was originally about were never a published copy of anything. They were
collected off a dirty working tree, which is the defect, and the `git ls-files` filter added on
2026-08-10 already prevents it.

With its last open question answered, P39 classifies CORRECTED. The live register count moves from
53 to 52, propagated to `SUMMARY.md`, `docs/CONTINUITY.md`, `docs/PROGRAMME_EXECUTION.md` and the
closure routing table. Dated audit lines in `docs/GEN5_CLOSURE.md` and the earlier
`PROGRAMME_EXECUTION.md` batch record are left at 53, because they record what was true on their
own dates. No band, no result and no Gen5 or Gen6 number changed.

...
At the rated point (3U reference, 9.445~kg sled computed from the CAD solid volumes, 1.3~m acceleration): exit velocity \textbf{16.0~m/s} at \textbf{10.1~g}, pulse 162~ms, peak current 320~A, and bank state-of-charge sag 5.2\% (Fig.~\ref{fig:shot}). Energy drawn is 2.78~kJ per shot, with copper heat of 854~J and a further 76~J dissipated in the bank's own series resistance. Regenerative braking returns 47~J after release, so the net draw is 2.74~kJ. The payload receives 514~J, a \textbf{18.8\% electrical-to-payload efficiency}; the sled's share is split between 47~J recovered and 1162~J dissipated in the arrest brake. Efficiency falls with the heavier sled for two compounding reasons: a larger fraction of the same mechanical work goes into a mass that is then braked away, and the longer 162~ms pulse accrues more copper loss at unchanged current density. Recovery is what partially offsets the first of those, and it is bounded by geometry rather than by loss: over the available 39~mm the braking pulse lasts 2.5~ms and burns 0.4~J of copper, against 854~J over the 162~ms shot, so recovered energy rises monotonically with braking force up to the winding's rating and with distance beyond it. A 500~mm section would return 540~J and a 1~m section would arrest the sled outright while returning 727~J, but the closed envelope already exceeds its target class by 44\% (Sec.~\ref{sec:lim}), so distance is not available here. Peak current during recovery is 212~A, below the shot's own 320~A, so the drive is not re-rated. The closed-loop Monte Carlo returns a dispersion of \textbf{0.0274~m/s (3$\sigma$)} at a 15.8~m/s fleet setpoint (Fig.~\ref{fig:mc}), dominated by the sensing chain rather than plant tolerances. The setpoint must sit below the open-loop ceiling for the servo to retain authority; at 98.85\% of it, the nominal headroom is 0.188~m/s. Table~\ref{tab:family} gives the payload family: the system is force-limited above 1U, so heavier payloads trade velocity, never structural margin, and the deployer mass each customer pays for varies by a factor of 109 across the family (Sec.~\ref{sec:family}).

...
about 2.8x short. I expect the section length to grow by the same factor and the mass with it,
so **band 4 fails**.
...
The dispersion tracks the friction work almost exactly, 3.57x against 3.67x, which is the
...
>
> So the energy the store must deliver grows 3.6x while the peak current it must switch is
...
rather than 3 %, the authority at 1.154 m/s rather than 0.9, the shortfall at 3.57x rather than

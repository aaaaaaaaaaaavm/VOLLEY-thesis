...
> energy and ESR x C of a film or pulse capacitor bank."*
...
> m >= ½, (ESR x C), P / (f, specific energy)

with the bus voltage cancelling exactly. So a technology change enters through only two terms:
ESR x C and specific energy, and a film capacitor moves the first by orders of magnitude.
...
723x to 1152x the energy the correction needs, purely to source the current.

A film capacitor's ESR x C is microseconds rather than seconds. If that holds, the power term
...
> 522x lighter at the worst corner, and 17x the specific power A54 said was required.
>
...
energy it delivers, purely to source the current. At a film capacitor's it holds 1.00x, the
...
bank because A10's ESR x C bracket and `mass_properties.py`'s 6.50 kg string were the only store
...
> ESR x C and specific energy, and named both as unsourced. This run sources them.*

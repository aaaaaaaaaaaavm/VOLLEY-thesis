...
[A10](A10_bank_esr.md) established that ESR x C is roughly constant within a cell technology,
...
<= 32.2 mΩ. [A10](A10_bank_esr.md)'s bracket of ESR x C then fixes the capacitance at
...
> Gen5's own bank achieves 4.72 kW/kg. *That is 4.92x on the axis that binds, and specific
...
> m >= ½, (ESRxC), P / (f, specific energy)

### Stroke does not enter either

Peak power is force-per-metre x exit velocity. Force per metre is fixed by A2's thrust constant
...
pulse capacitor's ESR x C is orders of magnitude lower, so the binding constraint flips from
power to energy, and then the question is specific energy, which is not in the record.
*NEEDS SOURCE: specific energy and ESR x C of a film or pulse capacitor bank at a 4.77 ms discharge.*

...
| A76 | Whether inclination through a variable atmosphere explains A75's residual | `pymsis` NRLMSISE-00, numpy around it | the named candidate in P79's residual | RUN 2026-09-15: bands 1, 3 and 6 pass, band 4 fails. **Inclination gives 1.0138x where A75's residual is 1.2428x**, the right sign and 6.3 % of it in log terms, short by 15.9x. Latitude carries almost all of that and the J2 nodal rate contributes 1.0002x. The point-wise latitude contrast is 1.109x to 1.345x, ample to explain the residual, but the gradient reverses sign with local solar time and orbit-averaging removes it. A first run that aliased its own sampling against the diurnal cycle is preserved at `f65eab3`; correcting it moved the answer by 0.0001. **P79's residual now has no named cause** |
